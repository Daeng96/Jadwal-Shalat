package com.akbar.prayertimes.c

import com.akbar.prayertimes.a.SimpleDate
import kotlin.math.floor

class UmmulQura {
    private val dayUmmQuro = arrayListOf(
        28607,
        28636,
        28665,
        28695,
    ...
    private fun gMod(l1: Double): Double {
        return ((l1 % 7) + 7) % 7
    }
     ...
     *  get day
     *
     *  val sd = SimpleDate(GregorianCalendar()
     *  val gc = getCalendarUmmQuro(sd)
     *
     *
     *  val day = gc[[0]]
     */
    fun getCalendarUmmQuro(date: SimpleDate): IntArray {
        val set = setCalendarUmmQuro(date)
        return set
    }

    fun getMonthString(date: SimpleDate): String? {
    ...
    private fun setCalendarUmmQuro(date: SimpleDate): IntArray {
        var day = date.day.toDouble()
        var mMonth = date.month.toDouble()
        val mYear = date.year.toDouble()
        var y = mYear
        var m = mMonth
        if (m < 3) {
            y -= 1
        ...
        var a = floor(y / 100.0)
        var jgc = a - floor(a / 4.0) - 2
        val cjdn = floor(365.25 * (y + 4716)) + floor(30.6001 * (m + 1)) + day - jgc - 1524
        a = floor((cjdn - 1867216.25) / 36524.25)
        jgc = a - floor(a / 4.0) + 1
        val b = cjdn + jgc + 1524
        var c = floor((b - 122.1) / 365.25)
        val d = floor(365.25 * c)
        mMonth = floor((b - d) / 30.6001)
        day = (b - d) - floor(30.6001 * mMonth)
        if (mMonth > 13) {
            c += 1
            mMonth - +12
        }
        mMonth -= 1
        val mcjdn = cjdn - 2400000
        val ccc = dayUmmQuro.size
        var i = 0
        while (i < ccc) {
            i++
            if (dayUmmQuro[i] > mcjdn) break
        }

        val iln = i + 16260
        val ii = floor((iln - 1) / 12.0)
        val iy = ii + 1
        val im = (iln - 12 * ii)
        val id = mcjdn - dayUmmQuro[i - 1] + 1
